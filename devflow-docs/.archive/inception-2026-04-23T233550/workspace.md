...
**Detected**: Brownfield
**Timestamp**: 2026-04-22T17:04:00+09:00
**Project Root**: /Users/jay.ahn/projects/infra/devflow-k8s-deploy
**Requires Path Confirmation**: false
**Source**: 이전 분석(2026-04-22T11:45:00+09:00) 기반 + 델타 업데이트 (Git Activity + validators 패키지 반영)

## Project Structure

v0.3.0 릴리스 완료된 Python 기반 K8s 배포 자동화 스킬. 스크립트 6,500+ 라인, 테스트 22개 파일(655 테스트), Claude Code 플러그인 구조.
...
- `scripts/validators/` — K8s 검증 규칙 패키지 (registry, core, rules/sec|res|img|life|sa|prb|svc)
- `scripts/stacks/jvm.py` — JVM 스택 구현
- `templates/manifest/` — deployment.tmpl, service.tmpl, serviceaccount.tmpl
- `templates/dockerfile/` — jvm.tmpl
- `tests/` — 22개 테스트 파일 (655 tests)
...
- **Recent Focus**: `devflow-docs/audit.md`, `scripts/pipeline/orchestrator.py`, `scripts/stacks/jvm.py`
- **Recent Commits**:
  - `d050480 Merge`: remote-tracking branch 'origin/main'
  - `8225911 refactor(validators)`: validate_k8s.py 규칙별 모듈 분리 — scripts/validators/ 패키지 (#21, #22)
  - `c48c9f6 fix(RES-W01)`: jvm small 티어 cpu_request 50m → 125m
  - `9a39019 feat(v0.3.0-p1)`: replicas 설정화 + LIFE-W01/IMG-W02 WARN 규칙 (#20)

## Existing Documentation

- `README.md` — 설치/사용법/제약/v0.3+ 로드맵
- `devflow-docs/backlog.md` — Next 5건(BL-001~005), Open 9건(BL-006~014)
...
- **Entry Points**: `scripts/pipeline/orchestrator.py` (SkillPipeline), `scripts/validate_k8s.py` (K8sValidator re-export)
- **Observed Patterns**: 레이어 구조 (pipeline → analyzer → stacks → templates), validators 규칙 레지스트리 패턴
