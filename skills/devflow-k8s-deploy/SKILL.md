...
사용자에게 한국어로 아래 6개 필드를 질문합니다. **각 질문마다 "?"를 입력하면 도움말을 표시**합니다. 상세 설명은 [## 도움말 카탈로그](#도움말-카탈로그-전체-f-02b-helpcatalog-10개-term)를 참조하세요.
...
"?" 입력 시 해당 term_id의 `ko_detail`을 표시합니다. 이 표가 단일 출처입니다.
...
| stateful | 상태성 앱이라는 게 뭐예요? (경고 발생 시) | DB 연결이나 파일 저장이 필요한 앱이에요. v0.1.0은 Deployment만 만들기 때문에, Pod 재시작 시 데이터가 사라질 수 있어요. v0.2부터 StatefulSet/PVC를 지원합니다. | `StatefulSet vs Deployment — Pod 재시작 시 데이터 보존` | stateless: 일반 API 서버 / stateful: DB, 메시지 큐, 파일 업로드 앱 | 2 |
