...
[STEP 3] manifests/deployment.yaml 생성 중...
[STEP 3] manifests/service.yaml 생성 중...
[STEP 3] manifests/serviceaccount.yaml 생성 중...
[STEP 3] 파일 생성 완료 → k8s-output/ (manifest는 manifests/ 서브디렉토리)
```
...
python "${CLAUDE_PLUGIN_ROOT}/scripts/validate_k8s.py" \
  --manifests {output_dir}/manifests/deployment.yaml {output_dir}/manifests/service.yaml {output_dir}/manifests/serviceaccount.yaml \
  --json
```
...
```bash
kubectl apply --dry-run=client --validate=false -f {output_dir}/manifests/
```
...
생성된 파일:
  ...
  {output_dir}/manifests/deployment.yaml
  {output_dir}/manifests/service.yaml
  {output_dir}/manifests/serviceaccount.yaml
...
다음 단계:
  kubectl apply -f {output_dir}/manifests/
