...
description: |
  JVM(Kotlin + Java Spring) 백엔드 프로젝트를 분석하여 Dockerfile + Kubernetes manifest를 생성합니다.
  "시니어 엔지니어의 판단력을 인코딩" — 비개발자/AI-assisted 사용자도 사용 가능.
  보안 디폴트(비root, readOnlyRootFilesystem, no-latest 태그, capabilities.drop=ALL) + 한국어 도움말.

  사용 시점:
  - "Dockerfile 만들어줘" / "k8s 배포 파일 만들어줘" / "컨테이너화해줘" 같은 요청
  - "배포 준비해줘" / "k8s에 올려줘" / "manifest 만들어줘" / "JVM 배포 설정해줘"
  - "deploy this to cluster" / "k8s manifest" / "Kubernetes 설정해줘"
  - JVM(Kotlin/Java Spring Boot) 프로젝트의 배포 자동화
...
JVM 프로젝트를 Kubernetes 배포용 Dockerfile + manifest로 자동 변환합니다. 시니어 엔지니어 수준의 보안 기본값과 한국어 도움말을 제공합니다.
...
> v0.1.0은 JVM 스택(Kotlin + Java Spring Boot)만 지원합니다. 다른 스택(Go, Python, React)은 v0.2+ 예정.
...
사용자에게 한국어로 아래 6개 필드를 질문합니다. **각 질문마다 "?"를 입력하면 도움말을 표시**합니다.
...
쿠버네티스에서 이 앱을 식별하는 이름입니다. 영문 소문자/숫자/하이픈만 사용하세요.
모르면 "?"를 입력하세요.
```

도움말("?" 입력 시):
> 앱 이름은 쿠버네티스에서 이 앱을 식별하는 라벨이에요. 보통 프로젝트 이름과 같게 짓고, 영문 소문자/숫자/하이픈만 사용합니다.
> 원어: `Deployment.metadata.name + Service.metadata.name + ServiceAccount.metadata.name`
> 예시: `my-api-service` / `order-backend`
...
Spring Boot 기본값은 8080입니다. application.yml에 server.port가 적혀 있으면 그 값을 쓰세요.
모르면 "?"를 입력하세요.
```

도움말("?" 입력 시):
> 앱이 요청을 받는 네트워크 포트예요. Spring Boot는 보통 8080입니다. `application.yml`에 `server.port`가 적혀 있으면 그 값을 쓰세요.
> 원어: `Container port + Service.spec.ports[].targetPort`
> 예시: `8080` (Spring Boot 기본) / `9000` (커스텀)
...
```
어디서 접속할 건가요?
  (a) 사내 네트워크만 (ClusterIP — 다른 앱끼리만 호출)
  (b) 외부 인터넷 공개 (LoadBalancer — 클라우드 비용 발생, 월 $20+)
  (c) 노드 포트 직접 (NodePort)
모르면 "?"를 입력하세요.
```
...
> - (a) ClusterIP: 사내 네트워크만 (다른 앱끼리만 호출) — 일반 백엔드 API에 권장
> - (b) LoadBalancer: 외부 인터넷 공개 (클라우드 비용 발생, 월 $20+) — 모바일/웹 공개 서비스
> - (c) NodePort: 노드 포트 직접 노출 (포트 번호 명시 권장)
> 원어: `Service.spec.type — ClusterIP / NodePort / LoadBalancer`
> 예시: `백엔드 API: ClusterIP` / `모바일 앱 백엔드: LoadBalancer`
...
'default'는 사고 방지를 위해 자동 배정하지 않습니다.
모르면 "?"를 입력하세요.
```

도움말("?" 입력 시):
> 네임스페이스(namespace)는 쿠버네티스에서 앱들을 분류하는 폴더 같은 개념이에요. 보통 프로젝트나 팀 이름을 씁니다. 'default'는 사고 방지를 위해 자동 배정되지 않아요.
> 원어: `Kubernetes Namespace — 리소스 격리 + RBAC 경계`
> 예시: `my-team` / `payment-svc` / `dev-jay`
...
도움말("?" 입력 시):
> Dockerfile과 yaml 파일이 만들어질 폴더예요. 기본은 `k8s-output/`이고, 이미 있으면 덮어쓸지 다시 물어봅니다.
> 원어: `Output directory (config: output.dir)`
> 예시: `k8s-output` (기본) / `deploy/k8s`
...
```
메모리/CPU는 어느 정도 필요해요?
  (a) small  — 256Mi / 0.5CPU (경량 서비스)
  (b) medium — 512Mi / 1CPU  (일반 Spring Boot) ← 권장
  (c) large  — 1Gi   / 2CPU  (고부하 서비스)
모르면 "?"를 입력하세요.
```

도움말("?" 입력 시):
> 앱이 사용할 자원을 추정해주세요. JVM은 기본 메모리 512Mi~1Gi를 추천합니다. 잘 모르겠으면 'medium'을 고르세요.
> 원어: `spec.containers[].resources.{requests,limits}.{cpu,memory}`
> 예시: `small (256Mi/0.5CPU)` / `medium (512Mi/1CPU)` / `large (1Gi/2CPU)`
...
1. **스택 감지**: `build.gradle.kts` / `build.gradle` / `pom.xml` 존재 여부로 JVM 확인
2. **빌드 시스템**: Gradle KTS / Gradle Groovy / Maven 판별
3. **Spring Boot 버전**: `spring-boot` 버전 추출 (2.x / 3.x)
4. **포트 추론**: `SERVER_PORT` 환경변수 → `application-{profile}.yml` → `application.yml` → 기본 8080 순서
5. **actuator 감지**: `spring-boot-starter-actuator` 의존성 여부
6. **multi-module 감지**: `settings.gradle(.kts)` 또는 Maven `<modules>` 블록
7. **상태성 감지**: DB 연결 / 파일 쓰기 / PVC 시그널 감지
...
```json
{
  ...
  "artifacts": {
    "dockerfile": "k8s-output/Dockerfile",
    "deployment": "k8s-output/deployment.yaml",
    "service": "k8s-output/service.yaml",
    "serviceaccount": "k8s-output/serviceaccount.yaml"
  }
}
```
...
Claude는 내부적으로 `${CLAUDE_PLUGIN_ROOT}/scripts/pipeline/orchestrator.py`를 통해 파이프라인을 실행합니다.
...
Claude가 bash로 실행할 때는 `${CLAUDE_PLUGIN_ROOT}`를 플러그인 루트 절대 경로로 치환하여 사용합니다 (NFR-05).

---

## 도움말 카탈로그 전체 (F-02b, HelpCatalog 10개 term)

| term_id | ko_short | step |
|---------|---------|------|
| app_name | 앱 이름은 뭘로 할까요? | 1 |
| port | 앱이 어떤 포트를 쓰나요? | 1 |
| exposure | 어디서 접속할 건가요? | 1 |
| namespace | 네임스페이스는 뭘로 할까요? | 1 |
| output_dir | 생성 파일을 어디에 둘까요? | 1 |
| resource_hint | 메모리/CPU는 어느 정도 필요해요? | 1 |
| actuator | actuator를 쓰고 있나요? | 2 |
| multi_module | 여러 모듈 중 어느 걸 배포할까요? | 2 |
| stateful | 상태성 앱이라는 게 뭐예요? (경고 발생 시) | 2 |
| build_engine | 이미지를 직접 빌드할까요? | config |

---

## v0.1.0 제약 및 v0.2+ 로드맵

### v0.1.0 제약
- JVM 스택만 (Kotlin + Java Spring Boot). Go/Python/React는 v0.2+ 로드맵
- `StatefulSet` 미지원 (Deployment만). stateful 앱 감지 시 경고 후 계속
- auto-fix 루프 v0.2+ (v0.1.0은 첫 실패 시 troubleshoot.md 안내 후 수동 수정 지시)
- PVC/NetworkPolicy/PDB 없음
- `buildah` / `kaniko` / `buildctl` 미지원 (v0.2+)
